...
- **Document source** (a hand-authored plan/backlog file holds many issues — tell by a finding citing a path like `PLAN.md:42`, or `format:"document"` in the config): `ac patch` and title/body edits still splice into the file at the issue's recorded span (top-level leaf items only — a nested item or one with id-bearing children fails closed too). State, assignee, label, parent/children, comments, writes to the umbrella issue, and delete all **fail closed** (the error names the file). For those, edit the document directly at the cited line, then re-run check — that's the sanctioned path, not a workaround.
