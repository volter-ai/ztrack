#!/usr/bin/env bash
# Claude Code Stop hook: the agent's turn cannot end while `tracker check`
...
set -uo pipefail
out="$(bunx ztrack check 2>&1)"
code=$?
if [ "$code" -eq 0 ]; then
  exit 0
fi
{
  echo "tracker check is failing — produce the missing evidence or fix the findings before ending the turn:"
