...
`,
  ...
  // (ZTB-1) covers the OUTSIDE case; this is the section's own interior blind spot. simple-gh-sdlc
  // duplicates parseDefaultIssue's grammar (see simple-sdlc.test.ts for the twin coverage).
  ...
  describe('input.root reads tolerate garbage (ZTB-38)', () => {
    ...
    // bundle, never a raw root). defaultPrBranches/citedEvidenceFiles/citedCommits must extract
    // facts BEST-EFFORT instead of throwing — exercised here via their observable effect on
    // ctx.git.prs / evidenceBlobs / commitFiles (where the pre-fix code threw
    // `...issues.flatMap is not a function`, or `.map` on a non-array `issues`).
    let bareDir: string;
    beforeAll(() => { bareDir = mkdtempSync(join(tmpdir(), 'ztrk-preset-ghsdlc-bare-')); });
    ...
    for (const bad of GARBAGE) {
      test(`loadContext does not throw on garbage root ${JSON.stringify(bad)}`, async () => {
        ...
        expect(result.git?.prs).toEqual({});
        expect(result.git?.evidenceBlobs).toEqual({});
        expect(result.git?.commitFiles).toEqual({});
      });
    }

    test('well-formed root: defaultPrBranches/citedEvidenceFiles/citedCommits resolve identically to before (real commit+branch fixture)', async () => {
      const repo = mkdtempSync(join(tmpdir(), 'ztrk-preset-ghsdlc-repo-'));
      try {
        ...
        const root: DefaultRoot = {
          issues: [{
            id: 'ZT-1', title: 'x', summary: '', status: 'in-review', assignee: 'otto', pr: { url: 'feature-x' },
            acceptanceCriteria: [{
        ...
        expect(result.git?.prs?.['feature-x']?.headSha).toBe(sha);
