...
// launch the develop/review agent on Termfleet -> wait for the issue's state to
...
//
//   bun pm-cycle.ts --repo <projectRepo> [--url <termfleet>] [--max-min 15]
...
function dispatch(role: 'develop' | 'review', id: string): string {
  ...
  const out = tf('claude', 'new', '-y', '--name', `${role}-${id}`, '--cwd', REPO, '--prompt-file', promptFile);
  return /"terminalSession":\s*"([^"]+)"/.exec(out)?.[1] ?? '(unknown)';
}
...
async function main() {
  ...
  while (true) {
    ...
    const session = dispatch(role, id);
    log(`  launched ${role} in ${session}`);
