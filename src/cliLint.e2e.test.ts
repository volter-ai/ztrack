...
describe('ztrack lint: audible summary line (plain text)', () => {
  ...
  beforeAll(() => {
    ...
    expect(zt(['issue', 'create', '--title', 'Clean case', '--label', 'type:case', '--state', 'draft', '--assignee', 'me', '--body', '# x\n\nNothing suspicious here.\n']).code).toBe(0);
  }, 30_000);
...
describe('ztrack lint: audible summary line (findings present)', () => {
  ...
  beforeAll(() => {
    ...
    expect(zt(['issue', 'create', '--title', 'Suspicious case', '--label', 'type:case', '--state', 'draft', '--assignee', 'me', '--body', body]).code).toBe(0);
  }, 30_000);
  afterAll(() => { if (mine) rmSync(mine, { recursive: true, force: true }); });
  ...
  test('--fail-on-warn still exits nonzero exactly as before (exit-code contract unchanged)', () => {
    expect(zt(['lint', '--fail-on-warn']).code).toBe(1);
  });
