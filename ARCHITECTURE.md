...
| `core/audit.ts` | append-only audit log (`.volter/tracker/.audit.jsonl`, gitignored/per-clone); timestamps derived; `observeChanges` diffs a preset-validated snapshot against a baseline and logs each change. Driven from BOTH the CLI (after any mutating command, via `cliAudit.ts` — ztrack #19) and the visualizer server (per request, also catching edits made outside ztrack's affordances). Diff-based, so one central pass per caller suffices — no per-mutation-path instrumentation |
...
| `cliAudit.ts` | after a mutating CLI command, runs one `observeChanges` pass over the validated export → audit log (best-effort; classifies mutations from argv) |
