...
- the standalone `default` / `spec` / `speckit` presets (`src/presets/*.test.ts`), which are
  **not reachable through the shipped `ztrack` CLI** — so a unit test is their only possible
  coverage.
