...
- **Document source** (a hand-authored plan/backlog file holds many issues — tell by a finding citing a path like `PLAN.md:42`, or `format:"document"` in the config): use the same verbs. `ac patch` and title/body edits splice leaf-item content; state and one assignee splice the item's header metadata (including parent items). Labels, parent/children, comments, writes to the umbrella issue, delete, and body/title edits on an item with id-bearing children still **fail closed** with an error naming the file.
...
## Dialect lenses (a file in the repo's own idiom)

A file tracking work in its own idiom — `### KQ1` sections with `- **Status**: 🟢/🟡/🔴` bullets (`emoji-register`), or `- [x] **WS-A: title**` rosters (`checkbox-roster`) — can be tracked WITHOUT rewriting it. `ztrack check <file>` auto-detects the dialect, checks every issue through a read-only lens, and prints the exact adopt command with the name filled in (`ztrack import <file> --register --dialect <name>` — config-only; the file is never modified). Lens issues show in `issue list`/`check` with their own ids and true statuses but **report, never gate** (errors downgrade to warnings suffixed `dialect lens`); every write at one fails closed. Registration mutates tracker-config.json — run the offered command only when the user wants the file tracked, otherwise just relay it. To manage the file through ztrack later, `ztrack import <lens-file> --register` materializes it to the native grammar: grammar-legal ids kept verbatim, hyphenless ones normalized (`KQ3` → `KQ-3`) with the old spelling recorded as a config alias that keeps resolving; requires `--register` because the file rewrite and the config upgrade are one stroke (`--dry-run` previews both).
